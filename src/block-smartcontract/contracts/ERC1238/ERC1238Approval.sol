...
contract ERC1238Approval {
    ...
    bytes32 private constant MINT_BATCH_APPROVAL_TYPEHASH =
        keccak256(
            "MintBatchApproval(address recipient,uint256[] ids,uint256[] amounts)"
        );

    // Domain Separator, as defined by EIP-712 (`hashstruct(eip712Domain)`)
    bytes32 public DOMAIN_SEPARATOR;

    event DomainSeparatorForMintApproval(
        string name, 
        string version, 
        uint256 chainId, 
        address verifyingContract, 
    ...
    constructor() {
        ...
        emit DomainSeparatorForMintApproval(
            eip712Domain.name, 
    ...
    function _verifyMintingApproval(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, mintApprovalHash)
        );
        // require(
        //     ecrecover(digest, v, r, s) == recipient,
